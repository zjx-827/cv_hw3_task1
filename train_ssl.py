...
def get_cifar100_data_loaders(download, shuffle=False, batch_size=256):
    train_dataset = datasets.CIFAR100('.', train=True, download=download, transform=transforms.ToTensor())

    train_loader = DataLoader(train_dataset, batch_size=batch_size, num_workers=0, drop_last=False, shuffle=shuffle)
    
    test_dataset = datasets.CIFAR100('.', train=False, download=download, transform=transforms.ToTensor())


    test_loader = DataLoader(test_dataset, batch_size=2*batch_size, num_workers=2, drop_last=False, shuffle=shuffle)
...
checkpoint = torch.load('./runs/Jun19_02-23-22_autodl-container-7850119152-2889f78b/checkpoint_0100.pth.tar', map_location=device)
...
optimizer = torch.optim.Adam(parameters, lr=0.0005, weight_decay=0.0008)
...
for epoch in range(epochs):
    ...
    writer.add_scalar('Accuracy/Top1_Train', top1_train_accuracy.item(), epoch)
    writer.add_scalar('Accuracy/Top1_Test', top1_accuracy.item(), epoch)
    writer.add_scalar('Accuracy/Top5_Test', top5_accuracy.item(), epoch)
